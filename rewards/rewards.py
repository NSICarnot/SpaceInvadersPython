...
class Rewards:
    ...
    def draw_container(self, screen: pygame.Surface) -> None:
        ...
        # Black effect
        pygame.draw.rect(screen, c.BLACK, (0, self.top_margin + self.left_margin, 4 * self.left_margin - 1, self.top_margin - 2 * self.left_margin))
        pygame.draw.rect(screen, c.BLACK, (c.WIDTH - 4 * self.left_margin - 1, self.top_margin + self.left_margin, 4 * self.left_margin, self.top_margin - 2 * self.left_margin))
...
class _Item:
    def __init__(self, image_path, name: str, rarity: int) -> None:
        self.item_image: pygame.Surface = pygame.image.load(image_path)
        self.name = name
        self.rarity = rarity
